...
import {Backdrop, CloseButton, ColorPicker} from "@/components/tag/TagModals";

export default function TagFormModal({ mode, initial, onClose, onSubmit }: {
    mode: "create" | "edit";
    initial?: Tag;
    onClose: () => void;
    onSubmit: ({tag_id,name,description,color}:{ tag_id?: string, name: string, description?: string, color?: string
    }) => void;
}) {
    ...
    {
        onSubmit({tag_id:initial?.tag_id,name:name,description:description,color:color})
    }
