...
/**
 * Snap a translation offset to the device pixel grid. The cursor-following
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
	...
	// ----- neighbor awareness: the other schedules in the SAME node -----
	...
	// neighbors cannot move under us.
	...
	/**
	 * Stop the gesture at the neighbor's edge. Runs AFTER snapping so the
	 * clamp always wins, and only against neighbors that sit clear of the
	...
	const createMoveOverlay = () => {
		...
		overlay.className =
			// physical left-0 anchor: the clone is positioned by translated from
