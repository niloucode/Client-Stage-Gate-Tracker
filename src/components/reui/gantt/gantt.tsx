...
interface UseGanttStateOptions<TData = unknown> extends GanttCallbacks<TData> {
	 ...
	 * node: "allow" (default), "clamp" to the neighbor's edge, or "reject".
...
function resolveSettings<TData>(
	...
	const {
		...
		// (excluded keys are intentionally unused — see eslint config:
		//  @typescript-eslint/no-unused-vars ignoreRestSiblings for reui)
		events: _e,
		defaultEvents: _de,
		scale: _v,
		defaultScale: _dv,
	...
	return {
		...
		// locale-first default: a de/for locale gets Monday weeks without also
...
function createGanttStore<TData>(
	...
	const setField = <K extends ControlledKey>(
		...
		if (key === "date" || key === "scale") {
			...
			// controlled path would mutate without notify) nor drop infinite - scroll growth for a navigation that never happened
...
interface GanttViewConfig<TData = unknown> {
	 ...
	 * clickable parts and drive your own creation flow from the slot.
	 */
