...
function GanttBar<TData = unknown>({
	...
	const consumerOwnsContent =
		children !== undefined || !!viewConfig.renderEvent;
		...
		<>
			{segment.isStart && (
				<span
					...
					className="absolute inset-y-0 inset-s-0.5 flex w-2 cursor-ew-resize items-center justify-start opacity-0 group-hover/gantt-bar-group:opacity-100 pointer-coarse:opacity-100"
			...
			{segment.isEnd && (
				<span
					...
					// indicator reads as "resize this end", not a centered pill.
					// pointer-coarse keeps it visible on touch, where hover never fires
					className="absolute inset-y-0 inset-e-0.5 flex w-2 cursor-ew-resize items-center justify-end opacity-0 group-hover/gantt-bar-group:opacity-100 pointer-coarse:opacity-100"
			...
			<>
				{progress !== null && (
					...
					<span
						...
						className="pointer-events-none absolute inset-y-0 inset-s-0 border-e border-(--gantt-event-color)/65 bg-(--gantt-event-color)/40 data-full:border-e-0"
