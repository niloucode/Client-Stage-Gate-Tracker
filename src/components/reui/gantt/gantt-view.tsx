...
interface TimelineRowBars {
	...
	/** Mode this row was packed under; the draft overlay must honor it. */
...
function GanttView({
	...
	/** Resource id -> every descendant id, for parent rollup. */
	...
	const baseRowBars = useMemo(() => {
		...
		// for the parent rollup (never O(rows x occurrences))
	...
	const clampContainer = (width: number, container: number) => {
		...
		// The tree's own minWidth is a PREFERENCE, not a license to squeeze the
	...
	// Layout effect, not affect: the first measure must flush BEFORE the first
				...
				<div className="flex h-8 border-b">
					...
					{viewConfig.columnsMenu && (
						<div
							...
							className="bg-background before:from-background sticky inset-e-0 z-10 flex h-full shrink-0 items-center ps-1.5 pe-2.5 before:pointer-events-none before:absolute before:inset-before:-inset-s-5rt-5 before:w-5 before:bg-linear-to-l before:to-transparent rtl:before:bg-linear-to-r"
						>
							{viewConfig.columnsMenu}
						</div>
					)}
				...
				<div className="relative h-8 border-b">
					<div className="flex h-full">
						{groups.map((group) => (
							<div
								...
								// sticky label's scroll port and the stick would never fire
                  ...
                  so the gutter is identical parked or pinned. */}
								<span
									...
									className="sticky inset-s-3 max-w-full truncate"
								>
									{group.label}
								</span>
							</div>
	...
	const horizontalScrollbar = (
		<ScrollBar
			...
			className="bg-background border-t-border! inset-e-0! z-40 h-4! rounded-none py-1"
		/>
	);
				...
				{treeConfig.resizable ? (
					<div
						...
						className={cn(
							...
							"after:absolute after:inset-y-0 after:-inset-s-after:-inset-e-1-1",
						)}
				...
				>
					{viewConfig.zoomControl && (
						<div
							...
							className="bg-background absolute inset-e-3 bottom-5 z-40 flex flex-col rounded-md border shadow-sm"
						>
                  ...
                  must still show at the zoom limits */}
					...
					{customScrollbars ? (
						...
						// behind the header and the thumb are clipped) and stops above the
						...
						<ScrollArea
							key={scale}
							className="h-full *:data-[orientation=vertical]:top-16.25! *:data-[orientation=vertical]:bottom-4! [&>*[data-orientation=vertical]>[data-orientation=vertical]]:h-auto!"
						>
...
function GanttCustomDragLayer() {
	...
	return (
		<div
			...
			data-custom=""
			// physical left-0 anchor: positioned by the engine's translated from
	...
	return (
		<div
			...
			onPointerMove={(e) => {
				...
				// it is never quantized to the interval grid. Its position is written
			...
			onClick={(e) => {
				...
				// committed nothing and the row felt dead. Treat it as a creation at the
		...
		>
					...
					>
						{viewConfig.renderSummary ? (
							...
							viewConfig.renderSummary({
								...
								start: new Date(
									rangeStartMs +
										bars.summary.from * (rangeEndMs - rangeStartMs),
								),
								end: new Date(
							...
							<>
								...
								<span
									aria-hidden
									className="bg-muted-foreground/50 absolute inset-s-0 top-1/2 h-3 w-0.5 -translate-y-1/2 rounded-full"
								/>
								<span
									aria-hidden
									className="bg-muted-foreground/50 absolute inset-e-0 top-1/2 h-3 w-0.5 -translate-y-1/2 rounded-full"
								/>
								<div className="bg-muted-foreground/20 relative h-1.5 overflow-hidden rounded-full">
									{bars.summary.progress !== null && (
										<div
											...
											className="bg-muted-foreground/50 absolute inset-y-0 inset-s-0 rounded-full"
											style={{ width: `${bars.summary.progress}%` }}
										/>
									)}
								</div>
								{bars.summary.progress !== null && (
									<span className="text-muted-foreground absolute inset-s-full top-1/2 ms-2 -translate-y-1/2 whitespace-nowrap">
				...
				{ghost && (
					...
					>
						{ghost.kind !== "move" && (
							...
							<span className="pointer-events-none absolute inset-s-full top-1/2 ms-2 max-w-60 -translate-y-1/2 truncate whitespace-nowrap">
								{ghost.title}
							</span>
						)}
					</div>
			...
			{draft && draftLabel && (
				<div
					...
					style={{
						...
						// the schedule on the track above - and the one thing a creation
				...
				>
					<span className="bg-foreground text-background absolute inset-s-full top-1/2 ms-2 inline-flex w-max -translate-y-1/2 items-center rounded-md px-2 py-1 text-xs font-medium whitespace-nowrap">
						{draftLabel}
                ...
                arrow reads the same weight as the ones above and below. */}
						<span
							aria-hidden
							className="bg-foreground absolute inset-s-0 top-1/2 size-1.5 -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-[1px]"
						/>
					</span>
				</div>
			)}
          ...
          a free point, not a click. */}
			{hintVisible && hintStop && (
				<div
					data-slot="gantt-schedule-hint"
					// rtl flips the translation sign: insetInlineStart anchors the box's
				...
				>
					{viewConfig.renderScheduleHint ? (
						...
						// the content drives its own creation flow
						viewConfig.renderScheduleHint({
              ...
              nearest boundary. */}
							...
							<span
								...
								className={cn(
									"bg-foreground text-background absolute inset-s-1/2 inline-flex w-max -translate-x-1/2 items-center rounded-md px-2 py-1 text-xs font-medium whitespace-nowrap",
							...
							>
								...
								<span
									aria-hidden
									className={cn(
										"bg-foreground absolute inset-s-1/2 size-2.5 -translate-x-1/2 rotate-45 rounded-xs",
				...
				{chips.map((chip) => (
					<Tooltip key={`${chip.id}-${chip.side}`}>
						...
						>
							...
							<span
								aria-hidden
								className="ring-background absolute -inset-e-px -top-px size-1.5 rounded-full ring-1"
