...
import { createPhase, updatePhase, cascadeSoftDeletePhase } from "./phaseActions";
...
export function useUpdatePhase() {
  ...
  return useMutation({
    mutationFn: (params: { phaseId: string; stageId: string } & PhaseUpdateInput) =>
      updatePhase(
        params.phaseId,
        params.name,
        params.description,
...
export function useSwapPhaseOrder() {
  ...
  return useMutation({
    mutationFn: (params: { phaseId1: string; phaseId2: string; stageId: string }) =>
