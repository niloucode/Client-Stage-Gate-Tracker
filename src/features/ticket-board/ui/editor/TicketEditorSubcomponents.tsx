...
import { IssueTableModal, mapLinkedIssueChip } from "@/entities/issue";
...
function StatusBadge({ status }: { status: StatusEnum }) {
	...
	return (
		<span
			className={`inline-flex items-center gap-1 text-xs ${config.textClass}`}
		>
					...
					{filteredTickets.length === 0 ? (
						<div className="py-10 text-center text-muted-foreground flex flex-col items-center justify-center space-y-2">
							...
							<p className="text-xs mt-1">
								All other tickets are either subtasks already or finished.
							</p>
						</div>
					) : (
										...
										<div className="flex items-center gap-2 text-xs text-muted-foreground truncate mt-1">
											<span>
												{ticket.plan_start_at
													? new Date(ticket.plan_start_at).toLocaleDateString()
													: "No start date"}
											</span>
											<span>•</span>
		...
		<div className="flex flex-col gap-3 px-5 py-3.5 border-b border-gray-100 shrink-0 relative">
			<div className="flex items-center justify-between gap-3 max-w-full">
				<div className="inline-flex items-center gap-2 max-w-full min-w-0 flex-1">
					<input
						...
						maxLength={50}
						onChange={(e) =>
							setTicketAction((t) => ({ ...t, name: e.target.value }))
						}
				...
				<DropdownMenu>
					...
					<DropdownMenuContent className="w-44" align="end">
						<DropdownMenuGroup>
							...
							<DropdownMenuRadioGroup
								value={ticket.status}
								onValueChange={(val) => setStatus(val as StatusEnum)}
							>
								{STATUSES.map((s) => (
									<DropdownMenuRadioItem
										key={s}
									...
									>
										<span className="flex items-center gap-2">
											...
											/>
											<span
												className={`text-sm font-medium ${STATUS_CONFIG[s].textClass}`}
											>
			...
			<div className="flex items-center justify-between gap-2 w-full">
				...
				<DropdownMenu>
					...
					<DropdownMenuContent
						className="w-64 max-h-52 overflow-y-auto"
						align="end"
					>
						<DropdownMenuGroup>
							<DropdownMenuLabel className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-2 py-1">
...
export function TicketAssignees({
	...
	const availableProfiles = profiles.filter(
		(u) =>
			!(ticket.TicketAssigned || []).some((a) => a.profile_id === u.profile_id),
	);
		...
		<div className="grid grid-cols-2 gap-x-6 gap-y-4 items-start">
			<div>
				<div className="flex items-center gap-2 mb-1.5 h-8">
					<Label className="my-auto text-xs text-neutral-border font-bold">
						ASSIGNED TO
					</Label>
				...
				<div className="flex flex-wrap items-center gap-1.5">
					{hasAssignees ? (
						ticket.TicketAssigned.map((a) => {
							const fullName =
								`${a.Profile?.first_name ?? "Unknown"} ${a.Profile?.last_name ?? "User"}`.trim();
							return (
								<div
									key={a.profile_id}
									title={fullName}
					...
					<DropdownMenu>
						...
						>
							<DropdownMenuGroup>
								...
								<DropdownMenuItem
									onClick={() =>
										setTicketAction((t) => ({ ...t, watcher_id: null }))
									}
				...
				<div className="flex flex-wrap items-center gap-1.5 h-8">
					{watcher ? (
						<div
							...
							className="group relative inline-flex items-center justify-center shrink-0 gap-2"
						>
							<UserAvatar
		...
		<div className="space-y-2">
			...
			<div className="grid grid-cols-[110px_1fr] gap-3">
				...
				<Input
					...
					value={apiRoute}
					onChange={(e) => setApiRouteAction(e.target.value)}
				/>
			</div>
		</div>
	);
...
export function TicketSchedule({
	...
	const [linkedIssue, setLinkedIssue] = useState<LinkedIssueChip | null>(() =>
		ticket.Issues ? mapLinkedIssueChip(ticket.Issues) : null,
	);
		...
		<div className="space-y-2 pt-1">
			...
			<div className="grid grid-cols-2 gap-4">
				<div>
					<DateTimePicker
						label="PLANNED START"
						value={
							ticket.plan_start_at ? new Date(ticket.plan_start_at) : undefined
						}
						onChange={(date) =>
							setTicketAction((t) => ({
								...t,
								plan_start_at: date ?? t.plan_start_at,
							}))
						}
				...
				<div>
					<DateTimePicker
						...
						required
						value={
							ticket.plan_end_at ? new Date(ticket.plan_end_at) : undefined
						}
						onChange={(date) =>
							setTicketAction((t) => ({
								...t,
								plan_end_at: date ?? t.plan_end_at,
							}))
						}
				...
				<div>
					<DateTimePicker
						label="FINISH"
						value={
							ticket.actual_end_at ? new Date(ticket.actual_end_at) : undefined
						}
						disabled
							...
							{linkedIssue && (
								<button
									...
									onClick={(e) => {
										e.stopPropagation();
										setLinkedIssue(null);
										setTicketAction((t) => ({ ...t, issue_id: null }));
									}}
			...
			<IssueTableModal
				...
				onSelectIssue={(issue) => {
					...
					setTicketAction((t) => ({ ...t, issue_id: issue.id }));
				}}
			/>
		</div>
	);
