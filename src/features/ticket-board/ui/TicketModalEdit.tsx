...
export default function TicketModalEdit({
	...
	async function handleSave() {
		...
		const updated = await updateTicketMutation.mutateAsync({
			...
			name: ticket.name,
			plan_end_at: ticket.plan_end_at ?? undefined,
					...
					<div className="px-5 py-4 grid grid-cols-2 gap-x-6 gap-y-4 border-b border-gray-100">
							...
							<div className="flex flex-wrap items-center gap-1.5">
								...
								{ticket.TicketAssigned && ticket.TicketAssigned.length > 0 ? (
									ticket.TicketAssigned.map((a) => {
										const firstName = a.Profile?.first_name ?? "Unknown";
										const lastName = a.Profile?.last_name ?? "User";
						...
						<div className="relative">
							...
							<div
								...
								onClick={() =>
									setEditing(editing === "watcher" ? null : "watcher")
								}
							>
								{watcher ? (
						...
						<div className="relative" ref={statusRef}>
							...
							<div className="flex items-center">
								<div className="h-1 w-1 mr-2 bg-brand-600"></div>
								<button
									onClick={() =>
										setEditing(editing === "status" ? null : "status")
									}
