...
export function PendingContracts({ contracts, role }: PendingContractsProps) {
	const router = useRouter();
	const myContracts = useMemo<PendingContract[]>(
		() =>
			(contracts ?? []).map((contract) => {
				const clientSigned = contract.client_signed_at
					? "Signed"
					: "Pending Signature";
				const projectOwnerSigned = contract.project_owner_signed_at
					? "Signed"
					: "Pending Signature";
				const projName = contract.Projects.name;
				return {
					id: contract.contract_id,
					documentName: contract.contract_name ?? `Contract for ${projName}`,
					projectName: projName,
					status: role === "Client" ? clientSigned : projectOwnerSigned,
					onReview: () => {
						router.push(
							`/projects/${contract.project_id}/contract?projectId=${contract.project_id}&clientId=${contract.client_id}`,
						);
					},
				};
			}),
		[contracts, role, router],
	);
